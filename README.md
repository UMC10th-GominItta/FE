...
```
Splash → Onboarding(GOAL) → [Login¹] → Home
                                          │
        ┌────── 하단 탭 바 (4탭) ──────────┼───────────┬───────────┐
        ▼                ▼                ▼           ▼
      홈             마음 세션         마음 레시피     리포트
   HomeScreen     SessionList        Recipe¹      Report²(미설계)
        │
        ├─[오늘의 고민 보관하러 가기]─▶ WorryInput ▶ WorryIntensity ▶ WorrySchedule ▶ WorryMemo ▶ WorrySaved ─▶ (홈 복귀)
        │
        ├─[다음 마음 세션 카드]───────▶ SessionDetail ▶ SessionActive ▶ SessionComplete ▶ SessionRating ─▶ (홈 복귀)
        │
        └─[우상단 프로필]────────────▶ MyPage / Settings

마음 세션 탭(SessionList) → 세션 항목 선택 → SessionDetail → …(위 마음 세션 플로우와 동일)
```
