...
> 구조 규칙: 각 화면은 `presentation/<feature>/<ScreenId>.kt`, 라우트는 `Routes.kt` 상수(`const val HOME = "home"` 식), 화면 간 이동은 `AppNavHost.kt` 에서 배선.

### 온보딩 · 인증

| 화면 이름       | 스크린 ID              | 용도                         | 진입 경로          | 담당자 | UI 구현 | 로직 연동 |
| ----------- | -------------------- | -------------------------- | -------------- | --- | --- | --- |
| 온보딩 · 스플래시  | `OnboardingScreen`   | 앱 소개 4페이지 가로 스와이프 페이저      | 스플래시 후 (최초 1회) | 강준형 | ✅ |   |
| 로그인         | `LoginScreen`        | 카카오 로그인 진입 화면              | 온보딩 후          | 강준형 | ✅ | 🚧 진행중 |
| 로그인 완료      | `LoginCompleteScreen` | 로그인 완료 안내                  | 로그인 → 자동       | 강준형 | ✅ |   |

### 메인 — 하단 탭 바 (4탭)

| 화면 이름      | 스크린 ID                | 용도                                               | 진입 경로          | 담당자 | UI 구현 | 로직 연동 |
| ---------- | --------------------- | ------------------------------------------------ | -------------- | --- | --- | --- |
| 홈          | `HomeScreen`          | 인사말 + "오늘의 고민 보관하러 가기" · "다음 마음 세션" 카드 + 오늘의 한마디 | 로그인 후 / 하단탭: 홈 | 강준형 | ✅ | 🚧 진행중 |
| 마음 세션      | `SessionListScreen`   | 예정 · 지난 세션 목록                                    | 하단탭: 마음 세션     | 장예윤 | ✅ |🚧 진행중 |
| 마음 레시피     | `RecipeCenterScreen` 등 | 마음 관리 콘텐츠 / 팁 라이브러리 (Create·Edit·Run 세부화면 포함)  | 하단탭: 마음 레시피    | 김세림 | ✅ | 🚧 진행중 |
| 리포트        | `ReportScreen`        | 고민 · 감정 통계 그래프 (탭 3종: 불안온도·걱정지도·타임라인)             | 하단탭: 리포트       | 조세환 | ✅ | 🚧 진행중 |
| 마이페이지 / 설정 | `MyPageScreen` 등      | 프로필 · 설정 · 즐겨찾는 시간 · 알림설정 · 탈퇴 등 세부화면 포함          | 홈 우상단 프로필 아이콘  | 김세림 | ✅ | 🚧 진행중 |

### 걱정 예약 플로우 — 홈 → "오늘의 고민 보관하러 가기"

| 화면 이름    | 스크린 ID                 | 용도                     | 진입 경로              | 담당자 | UI 구현 | 로직 연동 |
| -------- | ---------------------- | ---------------------- | ------------------ | --- | --- | --- |
| 걱정 입력    | `WorryInputScreen`     | 걱정 자유 입력                | 홈 → 고민 보관하러 가기     | 강준형 | ✅ | 🚧 진행중 |
| 걱정 정도 선택 | `WorryIntensityScreen` | 고양이 + 슬라이더로 걱정 강도 선택   | 걱정 입력 → 다음         | 강준형 | ✅ | 🚧 진행중|
| 걱정 시간 예약 | `WorryScheduleScreen`  | 걱정을 다시 생각 할 날짜/시간 선택   | 걱정 정도 → 다음         | 강준형 | ✅ | 🚧 진행중 |
| 한 줄 메모   | `WorryMemoScreen`      | 걱정 한 줄 메모               | 걱정 시간 예약 → 다음      | 강준형 | ✅ | 🚧 진행중|
| 걱정 보관 완료 | `WorrySavedScreen`     | 걱정 보관 확인                | 한 줄 메모 → 완료 → 홈 복귀 | 강준형 | ✅ | 🚧 진행중 |

> 5개 화면 모두 ViewModel 없이 로컬 `remember` 상태만 사용 — 입력한 걱정 내용이 다음 화면·홈으로 전달/저장되지 않음.

### 마음 세션 플로우 — 홈 "다음 마음 세션" 카드 / 마음 세션 탭 → 항목

| 화면 이름          | 스크린 ID                  | 용도                     | 진입 경로                 | 담당자 | UI 구현 | 로직 연동 |
| -------------- | ----------------------- | ---------------------- | --------------------- | --- | --- | --- |
| 마음 세션 상세 / 인트로 | `SessionDetailScreen`   | 보관한 걱정 표시 + 시작 / 건너뛰기  | 홈 다음세션 카드 · 세션목록 항목 탭 | 장예윤 | ✅ | 🚧 진행중 |
| 마음 세션 진행       | `SessionActiveScreen`   | 감정 선택 그리드 + 세션 완료하기    | 세션 상세 → 시작            | 장예윤 | ✅ | 🚧 진행중 |
| 마음 세션 완료       | `SessionCompleteScreen` | "마음 세션 완료" + 일상으로 돌아가기 | 세션 진행 → 완료            | 장예윤 | ✅ | 🚧 진행중 |
| 마음 세션 평가       | `SessionRatingScreen`   | 슬라이더 기분 평가 + 저장        | 세션 완료 → 저장 → 홈 복귀     | 장예윤 | ✅ | 🚧 진행중 |


### 마음 레시피 플로우 — 세션 "레시피 실행" 카드 / 마음 레시피 탭 → 항목

| 화면 이름    | 스크린 ID              | 용도                      | 진입 경로            | 담당자 | UI 구현 | 로직 연동 |
| -------- | -------------------- | ----------------------- | ---------------- | --- | --- | --- |
| 마음 레시피 홈 | `RecipeCenterScreen` | 레시피 목록 + 생성/실행/수정/삭제 진입 | 하단탭: 마음 레시피      | 김세림 | ✅ | 🚧 진행중 |
| 레시피 생성   | `RecipeCreateScreen` | 제목·설명·소요시간 입력 후 등록      | 레시피 홈 → 생성(+) 버튼 | 김세림 | ✅ | 🚧 진행중 |
| 레시피 실행   | `RecipeRunScreen`    | 선택한 레시피 진행               | 레시피 홈 → 항목 탭      | 김세림 | ✅ | 🚧 진행중 |
| 레시피 수정   | `RecipeEditScreen`   | 기존 레시피 수정 / 삭제          | 레시피 홈 → 수정 아이콘   | 김세림 | ✅ | 🚧 진행중|
